...
#ifndef _TIGER_HASH_H
#define _TIGER_HASH_H
...
static const uint64_t rhash_tiger_sboxes[4][256] = {
	{
 ...
 */
MAVLINK_HELPER void rhash_tiger_init(tiger_ctx* ctx)
{
	ctx->length = 0;
	ctx->tiger2 = 0;
...
#define t1Tiger rhash_tiger_sboxes[0]
#define t2Tiger rhash_tiger_sboxes[1]
#define t3Tiger rhash_tiger_sboxes[2]
#define t4Tiger rhash_tiger_sboxes[3]

#ifdef CPU_X64 /* for x86-64 */
#define roundTiger(a,b,c,x,mul) \
	c ^= x; \
	a -= t1Tiger[(uint8_t)(c)] ^ \
		t2Tiger[(uint8_t)((c) >> (2 * 8))] ^ \
		t3Tiger[(uint8_t)((c) >> (4 * 8))] ^ \
		t4Tiger[(uint8_t)((c) >> (6 * 8))] ; \
	b += t4Tiger[(uint8_t)((c) >> (1 * 8))] ^ \
		t3Tiger[(uint8_t)((c) >> (3 * 8))] ^ \
		t2Tiger[(uint8_t)((c) >> (5 * 8))] ^ \
		t1Tiger[(uint8_t)((c) >> (7 * 8))]; \
	b *= mul;
...
#define roundTiger(a,b,c,x,mul) \
	c ^= x; \
	a -= t1Tiger[(uint8_t)(c)] ^ \
		t2Tiger[(uint8_t)(((uint3Tiger2_t)(c)) >> (2 * 8))] ^ \
		t3Tiger[(uint8_t)((c) >> (4 * 8))] ^ \
		t4Tiger[(uint8_t)(((uint3Tiger2_t)((c) >> (4 * 8))) >> (2 * 8))] ; \
	b += t4Tiger[(uint8_t)(((uint3Tiger2_t)(c)) >> (1 * 8))] ^ \
		t3Tiger[(uint8_t)(((uint3Tiger2_t)(c)) >> (3 * 8))] ^ \
		t2Tiger[(uint8_t)(((uint3Tiger2_t)((c) >> (4 * 8))) >> (1 * 8))] ^ \
		t1Tiger[(uint8_t)(((uint3Tiger2_t)((c) >> (4 * 8))) >> (3 * 8))]; \
	b *= mul;
#endif /* CPU_X64 */
 ...
 */
static inline void rhash_tiger_process_block(uint64_t state[3], uint64_t* block)
{
 ...
 */
MAVLINK_HELPER void rhash_tiger_update(tiger_ctx* ctx, const unsigned char* msg, size_t size)
{
 ...
 */
MAVLINK_HELPER void rhash_tiger_final(tiger_ctx* ctx, unsigned char result[24])
{
