...
void inline digit_x_digit(digit_t a, digit_t b, digit_t *c)
...
__inline void fpsqr1271(felm_t a, felm_t c)
{ // Field squaring using schoolbook method, c = a^2 mod p

    fpmul1271(a, a, c);
}

__inline void mod1271(felm_t a)
...
__inline void mp_mul(const digit_t *a, const digit_t *b, digit_t *c, const unsigned int nwords)
{ // Schoolbook multiprecision multiply, c = a*b
    unsigned int i, j;
    digit_t u, v, UV[2];
    unsigned int carry = 0;
...
__inline void fpinv1271(felm_t a)
{ // Field inversion, af = a^-1 = a^(p-2) mod p
...
__inline unsigned int subtract(const digit_t *a, const digit_t *b, digit_t *c, const unsigned int nwords)
...
__inline void subtract_mod_order(const digit_t *a, const digit_t *b, digit_t *c)
...
__inline void add_mod_order(const digit_t *a, const digit_t *b, digit_t *c)
...
__inline void Montgomery_multiply_mod_order(const digit_t *ma, const digit_t *mb, digit_t *mc)
...
__inline void modulo_order(digit_t *a, digit_t *c)
...
__inline void conversion_to_odd(digit_t *k, digit_t *k_odd)
{ // Convert scalar to odd if even using the prime subgroup order r
    digit_t i, mask;
    digit_t *order = (digit_t *)curve_order;
    unsigned int carry = 0;
...
__inline void fp2div1271(f2elm_t a)
...
************************************************************************************/
__inline void table_lookup_1x8(point_extproj_precomp_t *table, point_extproj_precomp_t P, unsigned int digit, unsigned int sign_mask)
...
__inline void table_lookup_fixed_base(point_precomp_t *table, point_precomp_t P, unsigned int digit, unsigned int sign)
...
__inline void fp2copy1271(f2elm_t a, f2elm_t c)
{ // Copy of a GF(p^2) element, c = a
    fpcopy1271(a[0], c[0]);
    fpcopy1271(a[1], c[1]);
}

__inline void fp2zero1271(f2elm_t a)
{ // Zeroing a GF(p^2) element, a = 0
    fpzero1271(a[0]);
    fpzero1271(a[1]);
}

__inline void fp2neg1271(f2elm_t a)
{ // GF(p^2) negation, a = -a in GF((2^127-1)^2)
    fpneg1271(a[0]);
    fpneg1271(a[1]);
}

__inline void fp2sqr1271(f2elm_t a, f2elm_t c)
...
__inline void fp2mul1271(f2elm_t a, f2elm_t b, f2elm_t c)
...
__inline void fp2inv1271(f2elm_t a)
...
__inline void eccset(point_t P)
{ // Set generator
    // Output: P = (x,y)
...
__inline void eccnorm(point_extproj_t P, point_t Q)
...
__inline void R2_to_R4(point_extproj_precomp_t P, point_extproj_t Q)
{ // Conversion from representation (X+Y,Y-X,2Z,2dT) to (2X,2Y,2Z,2dT)
...
__inline void eccmadd_ni(point_precomp_t Q, point_extproj_t P)
{
    eccmadd(Q, P);
}

__inline void mLSB_set_recode(uint64_t *scalar, unsigned int *digits)
...
__inline bool ecc_mul_fixed(digit_t *k, point_t Q)
...
__inline bool ecc_mul_double(digit_t *k, point_t Q, digit_t *l, point_t R)
...
__inline void ecc_precomp_double(point_extproj_t P, point_extproj_precomp_t *Table, unsigned int npoints)
{ // Generation of the precomputation table used internally by the double scalar multiplication function ecc_mul_double().
...
__inline void wNAF_recode(uint64_t scalar, unsigned int w, int *digits)
...
__inline bool is_zero_ct(digit_t *a, unsigned int nwords)
{ // Check if multiprecision element is zero
    digit_t x;
    unsigned int i;
...
__inline void encode(point_t P, unsigned char *Pencoded)
{ // Encode point P
...
__inline ECCRYPTO_STATUS decode(const unsigned char *Pencoded, point_t P)
{ // Decode point P
    // SECURITY NOTE: this function does not run in constant time.
...
__inline void to_Montgomery(const digit_t *ma, digit_t *c)
{ // Converting to Montgomery representation

    Montgomery_multiply_mod_order(ma, (digit_t *)&Montgomery_Rprime, c);
}

__inline void from_Montgomery(const digit_t *a, digit_t *mc)
...
__inline void Montgomery_inversion_mod_order(const digit_t *ma, digit_t *mc)
...
__inline const char *FourQ_get_error_message(ECCRYPTO_STATUS Status)
{ // Output error/success message for a given ECCRYPTO_STATUS
...
__inline void ecc_psi(point_extproj_t P)
...
__inline void ecc_phi(point_extproj_t P)
...
__inline void decompose(uint64_t *k, uint64_t *scalars)
...
__inline void ecc_precomp(point_extproj_t P, point_extproj_precomp_t *T)
...
__inline void recode(uint64_t *scalars, unsigned int *digits, unsigned int *sign_masks)
...
__inline bool ecc_mul(point_t P, digit_t *k, point_t Q, bool clear_cofactor)
...
__inline void cofactor_clearing(point_extproj_t P)
{ // Co-factor clearing
    // Input: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates
...
MAVLINK_HELPER ECCRYPTO_STATUS CompressedPublicKeyGeneration(const unsigned char *SecretKey,
...
MAVLINK_HELPER ECCRYPTO_STATUS CompressedKeyGeneration(unsigned char *SecretKey,
                                                       unsigned char *PublicKey)
{
    // Keypair generation for key exchange. Public key is compressed to 32 bytes
...
MAVLINK_HELPER ECCRYPTO_STATUS CompressedSecretAgreement(const unsigned char *SecretKey,
...
// Input:  32-byte SecretKey
// Output: 32-byte PublicKey
MAVLINK_HELPER ECCRYPTO_STATUS SchnorrQ_KeyGeneration(const unsigned char *SecretKey, unsigned char *PublicKey)
{
    point_t P;
    unsigned char k[64];
    ECCRYPTO_STATUS Status = ECCRYPTO_ERROR_UNKNOWN;
...
// Outputs: 32-byte SecretKey and 32-byte PublicKey
MAVLINK_HELPER ECCRYPTO_STATUS SchnorrQ_FullKeyGeneration(unsigned char *SecretKey, unsigned char *PublicKey)
{
    ECCRYPTO_STATUS Status = ECCRYPTO_ERROR_UNKNOWN;
...
MAVLINK_HELPER ECCRYPTO_STATUS SchnorrQ_Sign(const unsigned char *SecretKey, const unsigned char *PublicKey, const unsigned char *Message, const unsigned int SizeMessage, unsigned char *Signature)
{
    point_t R;
...
MAVLINK_HELPER ECCRYPTO_STATUS SchnorrQ_Verify(const unsigned char *PublicKey, const unsigned char *Message, const unsigned int SizeMessage, const unsigned char *Signature, unsigned int *valid)
{
    point_t A;
